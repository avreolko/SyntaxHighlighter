...
@objc
public protocol HighlightDelegate {
    /**
     ...
     */
    @objc 
    optional func shouldHighlight(_ range:NSRange) -> Bool
     ...
     */
    @objc 
    optional func didHighlight(_ range:NSRange, success: Bool)
}
...
open class CodeAttributedString: NSTextStorage {
    ...
    open override func setAttributes(
        ...
        self.edited(
            TextStorageEditActions.editedAttributes,
            range: range,
            changeInLength: 0
        )
    }
    ...
    func highlight(_ range: NSRange) {
        ...
        if let highlightDelegate = highlightDelegate {
            let shouldHighlight = highlightDelegate.shouldHighlight?(range) ?? false
        ...
        DispatchQueue.global().async {
            let tempString = self.syntaxHighlighter.highlight(line, as: language)
            ...
            DispatchQueue.main.async(execute: {
                ...
                tempString?.enumerateAttributes(
                    in: NSMakeRange(0, (tempString?.length)!),
                    options: [], 
    ...
    func setupListeners() {
        syntaxHighlighter.themeChanged = { [weak self] _ in
